if (Meteor.isClient) {
  ...
  angular.module('uberShare-app').controller('UserAccess', function($http, $state){
    ...
    var postData = { "name": "eddie", 'email': email, 'password': password, 'password_confirmation': passwordconf};
    ...
    self.userLogin = function(email, password) {
      ...
      $http.get('https://stormy-falls-9947.herokuapp.com/users', postData, 'POST').then(function() {
        $state.go('profile');
      });
    }
