if (Meteor.isClient) {
  ...
  myApp.controller("notificationsCtrl", ['$scope', '$meteor', '$cookies',
    ...
    $scope.newMessages = function() {
      ...
      return Invites.find({to: user, unread: true}).fetch().length > 0
    };
  }]);

  // myApp.controller('notificationsCtrl', function($scope, $meteor, $cookies) {
  //   var self = this;
  //   self.newMessages = function() {
  //     var user = $cookies.get('username');
  //     Invites.find({to: user, unread: true}).fetch().length > 0
  //   }
  // });
